...
import siren.ocean.launcher.ui.AppDrawerAdapter;

public class CustomItemTouchHelper extends RecyclerView.ItemDecoration implements RecyclerView.OnChildAttachStateChangeListener {
    ...
    private final RecyclerView.OnItemTouchListener mOnItemTouchListener = new RecyclerView.OnItemTouchListener() {
        public boolean onInterceptTouchEvent(@NonNull RecyclerView recyclerView, @NonNull MotionEvent event) {
            ...
            if (action == 0) {
                ...
                CustomItemTouchHelper.this.select((RecyclerView.ViewHolder) null, 0);
            }
        ...
        public void onTouchEvent(@NonNull RecyclerView recyclerView, @NonNull MotionEvent event) {
            ...
            if (CustomItemTouchHelper.this.mActivePointerId != -1) {
                ...
                RecyclerView.ViewHolder viewHolder = CustomItemTouchHelper.this.mSelected;
                if (viewHolder != null) {
                    switch (action) {
                        case 2:
                            if (activePointerIndex >= 0) {
    ...
    void select(@Nullable RecyclerView.ViewHolder selected, int actionState) {
        if (selected != this.mSelected || actionState != this.mActionState) {
            ...
            if (actionState == 2) {
                if (selected == null) {
                    throw new IllegalArgumentException("Must pass a RecyclerView.ViewHolder when dragging");
                }
        ...
        this.mRecyclerView.post(new Runnable() {
            public void run() {
                if (CustomItemTouchHelper.this.mRecyclerView != null && CustomItemTouchHelper.this.mRecyclerView.isAttachedToWindow() && !anim.mOverridden && anim.mViewHolder.getAdapterPosition() != -1) {
                    RecyclerView.ItemAnimator animator = CustomItemTouchHelper.this.mRecyclerView.getItemAnimator();
                    if ((animator == null || !animator.isRunning((RecyclerView.ItemAnimator.ItemAnimatorFinishedListener) null)) && !CustomItemTouchHelper.this.hasRunningRecoverAnim()) {
            ...
            if (this.mActionState == 2) {
                ...
                if ((float) Math.abs(y - viewHolder.itemView.getTop()) >= (float) viewHolder.itemView.getHeight() * threshold || (float) Math.abs(x - viewHolder.itemView.getLeft()) >= (float) viewHolder.itemView.getWidth() * threshold) {
                    ...
                    if (swapTargets.size() != 0) {
                        String swap = "";
                        for (RecyclerView.ViewHolder i : swapTargets
                        ) {
    ...
    public void startDrag(RecyclerView.ViewHolder viewHolder) {
        if (!this.mCallback.hasDragFlag(this.mRecyclerView, viewHolder)) {
            ...
            bubbleDialog = ((AppDrawerAdapter.ViewHolder) viewHolder).bubbleDialog;
        }
    }
    ...
    void updateDxDy(MotionEvent ev, int directionFlags, int pointerIndex) {
        ...
        if (bubbleDialog != null) {
            if (Math.abs(mDx) > 40 || Math.abs(mDy) > 40) {
    ...
    private class ItemTouchHelperGestureListener extends SimpleOnGestureListener {
        ...
        public void onLongPress(MotionEvent e) {
            if (this.mShouldReactToLongPress) {
                ...
                if (child != null) {
                    RecyclerView.ViewHolder vh = CustomItemTouchHelper.this.mRecyclerView.getChildViewHolder(child);
                    if (vh != null) {
    ...
    public abstract static class Callback {
        ...
        public abstract boolean onMove(@NonNull RecyclerView var1, @NonNull RecyclerView.ViewHolder var2, @NonNull RecyclerView.ViewHolder var3);
        ...
        public RecyclerView.ViewHolder chooseDropTarget(@NonNull RecyclerView.ViewHolder selected, @NonNull List<RecyclerView.ViewHolder> dropTargets, int curX, int curY) {
            ...
            for (int i = 0; i < targetsSize; ++i) {
                RecyclerView.ViewHolder target = (RecyclerView.ViewHolder) dropTargets.get(i);
                int diff;
                int score;
                if (dx > 0) {
        ...
        void onDraw(Canvas c, RecyclerView parent, RecyclerView.ViewHolder selected, List<CustomItemTouchHelper.RecoverAnimation> recoverAnimationList, int actionState, float dX, float dY) {
        ...
        void onDrawOver(Canvas c, RecyclerView parent, RecyclerView.ViewHolder selected, List<CustomItemTouchHelper.RecoverAnimation> recoverAnimationList, int actionState, float dX, float dY) {
            int recoverAnimSize = recoverAnimationList.size();
